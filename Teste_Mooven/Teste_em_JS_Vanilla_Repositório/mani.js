(function() {
  ...
  const client_id = "Iv1.58338dd36f6c46c8";
  const client_secret = "bf3ac4b66cfc4bc9084f592735967475ffe6c7a3";
  const count = 7;
  ...
  async function getUser(user) {
    ...
    const reposResponse = await fetch(`${url}/${user}/respos?per_page=${count}&sort=${sort}&client_id=${client_id}&client_secret=${client_secret}`);
  ...
  function showRepos(repos){
    ...
    repos.forEach(repo => {
      ...
      <div class="row">
        <div class="col-md-6">
          <a href="${repo.html_url}" target="black">${repo.name}</a>
        </div>
        <div class="col-md-6">
          <span class="badge badge-primary">starts: ${
            repo.stargazers_count
          }</span>
          <span class="badge badge-success">watch: ${
            repo.watchers_count
          }</span>
          <span class="badge badge-warning">forks: ${
            repo.forks_count
          }</span>
        </div>
      </div>
